...
# =========================
# User settings (Spyder interactive)
# =========================
DATE_OLD: Optional[str] = None  # 在 Spyder 控制台预先定义，例如 DATE_OLD = "2025-09-12"
DATE_NEW: Optional[str] = None  # 同上，或运行时将提示输入
...
def _fetch_date_from_namespace(candidates: Tuple[str, ...]) -> Optional[str]:
    try:
        ...
        if ip is not None:
            ...
            for key in candidates:
                value = ns.get(key)
                if isinstance(value, str) and _is_valid_date_string(value):
                    return value
    except Exception:
        pass
    return None
...
def _resolve_date_from_inputs(namespace_candidates: Tuple[str, ...], env_candidates: Tuple[str, ...], prompt: str) -> str:
    value = _fetch_date_from_namespace(namespace_candidates)
    if value:
        return value
    for env_name in env_candidates:
        env_value = os.getenv(env_name)
        if isinstance(env_value, str) and _is_valid_date_string(env_value):
            return env_value
    while True:
        user_input = input(prompt).strip()
        if _is_valid_date_string(user_input):
            return user_input
        print("日期格式无效，请使用 YYYY-MM-DD。")


def _ensure_date(current: Optional[str], namespace_candidates: Tuple[str, ...], env_candidates: Tuple[str, ...], prompt: str) -> str:
    if isinstance(current, str) and _is_valid_date_string(current):
        return current
    return _resolve_date_from_inputs(namespace_candidates, env_candidates, prompt)
...
def main():
    global DATE_OLD, DATE_NEW
    DATE_OLD = _ensure_date(DATE_OLD, ("DATE_OLD", "date_old", "stock_date_old"), ("DATE_OLD", "INDEX_DATE_OLD"), "请输入旧日期（YYYY-MM-DD）：")
    DATE_NEW = _ensure_date(DATE_NEW, ("DATE_NEW", "date_new", "stock_date_new"), ("DATE_NEW", "INDEX_DATE_NEW"), "请输入新日期（YYYY-MM-DD）：")
