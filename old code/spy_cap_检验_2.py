# -*- coding: utf-8 -*-
"""
Created on Tue Jul 15 11:03:53 2025
...
# 2. 加载全市场数据（Excel 文件）
market_df = pd.read_excel("output/us_stock_us_spot_em.xlsx")
market_df.columns = market_df.columns.str.upper()

# 3. 清洗市场数据中的代码（去掉前缀）
market_df["代码_CLEAN"] = market_df["代码"].astype(str).str.upper().str.replace(r"^\d+\.", "", regex=True)
...
matched_df.to_excel(os.path.join(output_dir, "sp500_matched_market_data_em.xlsx"), index=False)
